<template>
    <div style="position: relative">
        <div class="captcha-img-out">
            <div class="captcha-img-panel" style="width: 400px;
                                                height: 200px;
                                                background-size: 400px 200px;
                                                margin-bottom: 1px;">
                <div class="captcha-refresh" style="z-index:3" @click="refresh">
                    <i class="iconfont icon-refresh"></i>
                </div>
                <img :src="'data:image/png;base64,' + originalImageBase64"
                    ref="canvas" alt="验证码图片" class="captcha-img" @click="handleClick" v-if="originalImageBase64">
                <img src="@/assets/img/loading.gif" alt="验证码加载中" class="captcha-img" v-else>
        ...
        <div class="captcha-bar-area" :style="{'color': captchaMsgColor, 'border-color': captchaMsgBorderColor}">
            <span class="captcha-msg">{{ wordList }}</span>
        </div>
    </div>
</template>
<script setup lang="ts">
import type { TempPoints } from '@/types/captcha'
import { getCaptchaApi, checkCaptchaApi } from '@/api/captcha'
const tempPoints = ref<TempPoints[]>([]);
const pointsArr = ref<TempPoints[]>([]);
const token =  ref<string>();
const wordList = ref<string>();
const originalImageBase64 = ref<string>();
const secretKey = ref<string>();
const captchaMsgColor = ref<string>();
const captchaMsgBorderColor = ref<string>();
const emit = defineEmits(['isCaptcha']);
const handleClick = (event: MouseEvent) => {
    // 获取点击事件相对于容器的位置
    const canvas = event.currentTarget as HTMLElement;
    const containerRect = canvas.getBoundingClientRect();                                                        
    const x = Math.round(310 * (event.clientX - containerRect.left)/parseInt('400px')); // x坐标
    const y = Math.round(155 * (event.clientY - containerRect.top)/parseInt('200px'));  // y坐标
...
const getCaptcha = async () => {
    tempPoints.value = [];
    pointsArr.value = [];
    captchaMsgColor.value = '#000000';
    captchaMsgBorderColor.value = '#ddd';
    originalImageBase64.value = '';
    const data = {
        "captchaType": "clickWord", // 验证码类型
    }
    const response = await getCaptchaApi(data);
    if (response.repCode === '0000') {
        token.value = response.repData.token;
        // wordList.value = resData.repData.wordList;
        originalImageBase64.value = response.repData.originalImageBase64;
        secretKey.value = response.repData.secretKey;
        wordList.value = '请依次点击【' + response.repData.wordList.join("，") + '】完成验证'
    } else {
        console.error('请求失败');
    }
}
...
const checkCaptcha = async () => {
    const data = {
        "captchaType": "clickWord", // 验证码类型
        "pointJson": secretKey.value ? aesEncrypt(JSON.stringify(pointsArr.value), secretKey.value) : JSON.stringify(pointsArr.value),
        "token": token.value,
    }
    const response = await checkCaptchaApi(data);
    if (response.repCode === '0000' && response.repData.result) {
        captchaMsgColor.value = '#4cae4c';
        captchaMsgBorderColor.value = '#4cae4c';
        wordList.value = "验证成功";
        setTimeout(() => {
            emit('isCaptcha', true);
        }, 300);
    } else if (response.repCode === '6111') {
        captchaMsgColor.value = '#d9534f';
        captchaMsgBorderColor.value = '#d9534f';
        wordList.value = "验证失败"
        setTimeout(() => {
            getCaptcha();
        }, 300);
    } else if (response.repCode === '6110') {
        captchaMsgColor.value = '#d9534f';
        captchaMsgBorderColor.value = '#d9534f';
        wordList.value = response.repMsg;
        setTimeout(() => {
            getCaptcha();
        }, 300);  
    }

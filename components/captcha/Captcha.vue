<template>
    <div class="mask" v-if="showCaptchaBox">
        <div class="captchabox" style="max-width: 430px;">
            <div class="captchabox-top">
                请完成安全验证
                <span class="captchabox-close" @click="closeCaptchaBox(false)">
                    <i class="iconfont icon-close"></i>
                </span>
            </div>
            <div class="captchabox-bottom">
                <!-- 验证码容器 -->
                <CaptchaPoints @isCaptcha = "isCaptcha" />
            </div>
        </div>
    </div>
</template>
...
const closeCaptchaBox = (isCaptcha : boolean) => {
    showCaptchaBox.value = false;
    emit('isshowCaptcha', false, isCaptcha);
};
const isCaptcha = (val: boolean) => {
    if (val) {
        closeCaptchaBox(true);
    }
};
watch(
  () => props.showCaptcha,
  (newVal, oldVal) => {
    showCaptchaBox.value = newVal;
    setTimeout(() => {
        showCaptchaBox.value = newVal;
    }, 500);
    //console.log(newVal);
